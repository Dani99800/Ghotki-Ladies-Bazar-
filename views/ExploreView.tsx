...
const ReelItem: React.FC<{ product: Product, muted: boolean, onBuy: () => void, onShop: () => void }> = ({ product, muted, onBuy, onShop }) => {
  ...
  return (
    <div className="relative h-screen w-full snap-start bg-black flex items-center justify-center overflow-hidden">
      ...
      <div className="absolute bottom-24 left-0 right-0 p-6 bg-gradient-to-t from-black/90 to-transparent z-20">
         <div className="flex items-end justify-between gap-4">
            <div className="flex-1 space-y-2">
               ...
               <h3 className="text-white font-black text-xl leading-tight tracking-tighter italic truncate">{product.name}</h3>
               <p className="text-pink-400 font-black text-lg italic">PKR {product.price.toLocaleString()}</p>
            </div>
            <button 
               onClick={(e) => { e.stopPropagation(); onBuy(); }}
               className="bg-white text-gray-900 font-black px-8 py-4 rounded-2xl flex items-center gap-3 shadow-2xl active:scale-95 transition-all text-[11px] uppercase tracking-widest"
            >
               Buy Now
            </button>
         </div>
      ...
      <div className="absolute bottom-32 right-4 flex flex-col gap-6 items-center z-30">
        ...
        <button className="p-4 bg-white/10 backdrop-blur-xl rounded-full text-white border border-white/10">
