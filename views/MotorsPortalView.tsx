...
interface MotorsPortalViewProps {
  products: Product[];
  shops: Shop[];
  user?: UserType | null;
...
const MotorsPortalView: React.FC<MotorsPortalViewProps> = ({ products, shops, user, addToCart }) => {
  ...
  // Filter products for Motors Portal
  const motorProducts = useMemo(() => {
    return products.filter(p => {
      const isMotorCat = p.category === 'Cars & Vehicles' || 
                         p.portal_type === 'MOTOR' || 
                         p.subcategory?.toLowerCase().includes('car') ||
                         p.subcategory?.toLowerCase().includes('motorcycle') ||
                         p.subcategory?.toLowerCase().includes('tyre') ||
                         p.subcategory?.toLowerCase().includes('auto');

      if (!isMotorCat) return false;
      ...
      const matchesMake = selectedMake === 'All' || 
                          p.vehicle_make?.toLowerCase() === selectedMake.toLowerCase() ||
                          p.name.toLowerCase().includes(selectedMake.toLowerCase());

      return matchesSearch && matchesSub && matchesLoc && matchesMake;
    });
  }, [products, shops, searchTerm, selectedSubCategory, selectedLocation, selectedMake]);

  const motorSubcategories = [
    { name: 'All', icon: Car },
    { name: 'Cars', icon: Car },
    { name: 'Motorcycles', icon: Gauge },
    { name: 'Tyres & Spare Parts', icon: Wrench },
  ];

  const makesList = ['All', 'Toyota', 'Honda', 'Suzuki', 'Yamaha'];

  return (
    <div className="min-h-screen bg-gray-50 pb-28">
      <PortalHeaderBar activePortal="MOTOR" />

      {/* Hero Banner */}
      <div className="bg-gradient-to-r from-amber-950 via-amber-900 to-orange-950 text-white p-6 md:p-10 relative overflow-hidden">
        <div className="absolute -right-10 -bottom-10 w-60 h-60 bg-amber-500/10 rounded-full blur-3xl"></div>
        <div className="max-w-7xl mx-auto relative z-10 flex flex-col md:flex-row items-start md:items-center justify-between gap-6">
          <div className="space-y-2">
            <div className="inline-flex items-center gap-2 px-3 py-1 bg-white/10 backdrop-blur-md rounded-full border border-white/20 text-amber-200 text-[10px] font-black uppercase tracking-widest">
              <Car className="w-3.5 h-3.5 text-amber-400" /> Motors & Vehicles Portal
            </div>
            <h1 className="text-2xl md:text-4xl font-black italic uppercase tracking-tighter">
              Ghotki & Sukkur Auto Market
            </h1>
            <p className="text-xs md:text-sm text-amber-200 max-w-xl font-medium leading-relaxed">
              Buy and sell verified cars, bikes, commercial vehicles, and original spare parts with instant seller contact & token deposit option.
            </p>
          </div>

          <button
            onClick={() => navigate(user ? '/seller' : '/login')}
            className="px-6 py-3.5 bg-amber-500 hover:bg-amber-400 text-white font-black text-[11px] uppercase tracking-widest rounded-2xl shadow-xl shadow-amber-950/50 flex items-center gap-2 active:scale-95 transition-all"
          >
            <PlusCircle className="w-4 h-4" /> Sell Your Car or Bike
          </button>
        </div>
      </div>
      ...
      <div className="max-w-7xl mx-auto p-4 space-y-4">
        ...
        <div className="bg-white p-3 rounded-2xl shadow-xs border border-gray-100 flex items-center gap-3">
          ...
          <input
            type="text"
            placeholder="Search Toyota, Honda Civic, Suzuki Alto, CD 70 or spare parts..."
            value={searchTerm}
        ...
        <div className="flex flex-wrap items-center justify-between gap-3 bg-white p-3.5 rounded-2xl border border-gray-100 text-xs">
          <div className="flex items-center gap-2 overflow-x-auto no-scrollbar py-0.5">
            <span className="font-black text-gray-400 uppercase text-[9px] tracking-wider shrink-0">
              Make / Brand:
            </span>
            {makesList.map((m) => (
              <button
          ...
          <div className="flex items-center gap-2 overflow-x-auto no-scrollbar py-0.5">
            <span className="font-black text-gray-400 uppercase text-[9px] tracking-wider flex items-center gap-1 shrink-0">
              <MapPin className="w-3 h-3 text-amber-600" /> Location:
            </span>
        ...
        <div className="space-y-3">
          ...
          {motorProducts.length === 0 ? (
            <div className="bg-white rounded-3xl p-12 text-center border border-gray-100 space-y-4">
              ...
              <div>
                <h3 className="font-black uppercase tracking-tight text-gray-900 text-lg">No Vehicles Found</h3>
                <p className="text-xs text-gray-400 mt-1 max-w-sm mx-auto">
                  Try clearing your make or location filter, or post a new vehicle ad!
                </p>
              </div>
              <button
                onClick={() => { setSearchTerm(''); setSelectedSubCategory('All'); setSelectedLocation('All'); setSelectedMake('All'); }}
                className="px-6 py-2.5 bg-amber-600 text-white font-bold text-xs rounded-xl shadow-md"
              >
                Reset All Filters
              </button>
            </div>
                    ...
                    <div className="p-5 flex-1 flex flex-col justify-between space-y-4">
                      <div className="space-y-2">
                        <div className="flex items-center justify-between text-[10px] text-gray-400 font-black uppercase tracking-wider">
                          ...
                          <span>{product.subcategory || 'Motors'}</span>
                        </div>
                        ...
                        <div className="flex flex-wrap gap-2 pt-1 text-[10px] font-bold text-gray-600">
                          {product.mileage_km && (
                            <span className="bg-gray-100 px-2.5 py-1 rounded-lg">🛣️ {product.mileage_km.toLocaleString()} KM</span>
                          )}
                          {product.fuel_type && (
                            <span className="bg-gray-100 px-2.5 py-1 rounded-lg">⛽ {product.fuel_type}</span>
                          )}
                          {product.transmission && (
                            <span className="bg-gray-100 px-2.5 py-1 rounded-lg">⚙️ {product.transmission}</span>
                          )}
                        </div>
                      </div>
                      ...
                      <div className="pt-3 border-t border-gray-100 flex items-center justify-between gap-3">
                        <div>
                          <p className="text-[9px] font-black uppercase tracking-widest text-gray-400">Demand Price</p>
                        ...
                        <div className="flex items-center gap-2">
                          <a
                            href={`https://wa.me/${whatsapp}?text=Hi, I am interested in buying vehicle: ${encodeURIComponent(product.name)}`}
                            target="_blank"
                          ...
                          <button
                            ...
                            className="px-4 py-3 bg-amber-600 text-white text-[10px] font-black uppercase tracking-wider rounded-2xl shadow-md hover:bg-amber-700 active:scale-95 transition-all"
                          >
                            Test Drive / Token
                          </button>
                        </div>
...
export default MotorsPortalView;
