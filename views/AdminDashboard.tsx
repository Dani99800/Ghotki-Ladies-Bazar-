...
import { 
  ...
  Sparkles, Palette, ChevronUp, ChevronDown, RefreshCw, ShoppingBag, LayoutGrid, Image as ImageIcon
} from 'lucide-react';
...
const AdminDashboard: React.FC<AdminDashboardProps> = ({ 
  ...
  const sortedShops = [...shops].sort((a, b) => {
    // 1. Pending shops at top for review
    if (a.status === 'PENDING' && b.status !== 'PENDING') return -1;
    if (a.status !== 'PENDING' && b.status === 'PENDING') return 1;
    
    // 2. Sort by Admin Priority
    ...
    <div className="max-w-4xl mx-auto p-4 space-y-8 pb-32 animate-in fade-in duration-500">
      ...
      <div className="flex gap-2 p-1.5 bg-gray-200 rounded-[2.5rem] overflow-x-auto no-scrollbar">
        {[
          ...
          <button 
            ...
            className={`flex-1 py-4 px-6 rounded-[2rem] text-[9px] font-black uppercase tracking-widest transition-all whitespace-nowrap flex items-center justify-center gap-2 ${activeAdminTab === tab.id ? 'bg-white text-pink-600 shadow-lg' : 'text-gray-500 hover:text-gray-700'}`}
          >
        ...
        <div className="bg-white p-8 rounded-[3rem] border border-gray-100 shadow-sm space-y-8">
          <div className="flex items-center justify-between px-2">
            <h2 className="font-black text-sm uppercase tracking-widest flex items-center gap-3 text-gray-900">
              <Store className="w-5 h-5 text-pink-500" /> Merchant Queue & Ranking
            </h2>
                ...
                <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-6">
                    ...
                    <div className="flex items-center gap-2 w-full md:w-auto">
                      ...
                      <div className="flex items-center bg-white rounded-xl p-1 border border-gray-100 shadow-sm">
                        <button 
                          onClick={() => movePriority(shop.id, shop.sort_priority || 0, 1)} 
                          disabled={loadingId === shop.id}
                          className="p-2 hover:bg-pink-600 hover:text-white rounded-lg transition-all disabled:opacity-50"
                      ...
                      <button 
                        ...
                        title="Toggle Top Seller Bag Badge"
                      >
                        {loadingId === shop.id ? <Loader2 className="w-5 h-5 animate-spin" /> : <ShoppingBag className="w-5 h-5" />}
                      </button>

                      {/* Status Toggle */}
                      <button 
                        onClick={() => updateStatus(shop.id, shop.status === 'APPROVED' ? 'SUSPENDED' : 'APPROVED')} 
                        disabled={loadingId === shop.id}
                        className={`px-4 py-3.5 rounded-xl text-[9px] font-black uppercase tracking-widest flex-1 md:flex-none ${shop.status === 'APPROVED' ? 'bg-red-50 text-red-600 border border-red-100' : 'bg-pink-600 text-white shadow-md'}`}
                      >
      ...
      {activeAdminTab === 'CATEGORIES' && (
        <div className="bg-white p-8 rounded-[3rem] border border-gray-100 shadow-sm space-y-8 animate-in slide-in-from-bottom-4">
          <div className="flex items-center justify-between px-2">
            <h2 className="font-black text-sm uppercase tracking-widest flex items-center gap-3 text-gray-900">
              <LayoutGrid className="w-5 h-5 text-pink-500" /> Marketplace Categories
            </h2>
            <button className="p-3 bg-pink-600 text-white rounded-2xl shadow-lg active:scale-95 transition-all">
               <Plus className="w-5 h-5" />
            </button>
          </div>
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            {categories.map((cat) => (
              <div key={cat.id} className="p-4 bg-gray-50 border border-gray-100 rounded-[2rem] flex items-center gap-4 group hover:bg-white hover:shadow-xl hover:border-pink-200 transition-all">
                ...
                <div className="flex-1">
                   <p className="font-black text-gray-900 uppercase italic tracking-tighter">{cat.name}</p>
                   <p className="text-[8px] font-black text-gray-400 uppercase tracking-widest">ID: {cat.id}</p>
                </div>
              </div>
            ))}
          </div>
      ...
      {/* Theme / Event Management Tab */}
      {activeAdminTab === 'THEME' && (
        <div className="bg-white p-8 rounded-[3rem] border border-gray-100 shadow-sm space-y-8 animate-in slide-in-from-bottom-4">
          <h2 className="font-black text-sm uppercase tracking-widest flex items-center gap-3 text-gray-900">
            <Sparkles className="w-5 h-5 text-pink-500" /> Active Application Theme
          </h2>
          <div className="grid grid-cols-2 sm:grid-cols-3 gap-3">
             {PK_EVENTS.map(event => (
                <button 
                  ...
                  className={`p-6 rounded-[2.5rem] border-2 transition-all text-left relative overflow-hidden group ${activeEvent.id === event.id ? 'border-pink-600 bg-pink-50 shadow-xl' : 'border-gray-50 bg-gray-50/30 hover:border-pink-100'}`}
                >
                   <span className="text-3xl block mb-2 transition-transform group-hover:scale-110">{event.emoji}</span>
                   <p className="font-black text-[9px] uppercase text-gray-900 tracking-tight">{event.name}</p>
                   <p className="urdu-font text-lg text-gray-400 opacity-60 mt-1">{event.urduName}</p>
                   {activeEvent.id === event.id && (
                     <div className="absolute top-3 right-3 bg-pink-600 text-white p-1 rounded-full shadow-lg">
