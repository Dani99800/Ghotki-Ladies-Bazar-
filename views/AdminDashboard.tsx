...
const AdminDashboard: React.FC<AdminDashboardProps> = ({ 
  ...
  // Sorting: Pending shops first, then by priority
      ...
      {activeAdminTab === 'THEME' && (
        <div className="bg-white p-8 rounded-[3rem] border border-gray-100 shadow-sm space-y-8 animate-in slide-in-from-bottom-4">
           <div className="flex items-center gap-3 px-2">
              ...
              <div>
                <h2 className="font-black text-sm uppercase tracking-widest text-gray-900 leading-none">Global Theme Switcher</h2>
                <p className="text-[8px] font-black text-pink-600 uppercase tracking-widest mt-1">Live Market Branding</p>
              </div>
           </div>
              ...
              {PK_EVENTS.map(event => (
                ...
                >
                  ...
                  <div className="relative z-10 space-y-4">
                    ...
                    <button className={`w-full py-3 rounded-2xl text-[9px] font-black uppercase tracking-widest transition-all ${activeEvent.id === event.id ? 'bg-gray-900 text-white' : 'bg-gray-100 text-gray-500'}`}>
                      {activeEvent.id === event.id ? 'Currently Active' : 'Activate Event'}
                    </button>
                  </div>
                </div>
              ))}
